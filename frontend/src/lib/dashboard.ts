...
export type DashboardData = {
  ...
  ultimosResponsaveis: ResponsavelResumo[]
}
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const [
    alunosResult,
    responsaveisCountResult,
    responsaveisListResult,
    registrosHojeResult,
  ...
  if (responsaveisCountResult.error) {
    throw new Error(`Erro ao buscar total de responsáveis: ${responsaveisCountResult.error.message}`)
  }

  if (responsaveisListResult.error) {
    throw new Error(`Erro ao buscar lista de responsáveis: ${responsaveisListResult.error.message}`)
  }

  if (registrosHojeResult.error) {
    throw new Error(`Erro ao buscar registros de hoje: ${registrosHojeResult.error.message}`)
  }

  if (registros7DiasResult.error) {
    throw new Error(`Erro ao buscar atividade dos últimos 7 dias: ${registros7DiasResult.error.message}`)
  }

  if (ultimosRegistrosResult.error) {
    throw new Error(`Erro ao buscar últimos registros: ${ultimosRegistrosResult.error.message}`)
  }
  ...
  return {
    ...
    totalResponsaveis: responsaveisCountResult.count || 0,
